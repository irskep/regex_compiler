...
class ASTNode(object):
    """
    A basic node which maintains all basic attributes and passes control down the AST generically in C{generate_instructions()}
    
    @ivar node_type: Label to put on a node in the graph, and to check type if necessary
    @ivar subtype: The particular production which was matched
    @ivar children: Ordered list of child nodes
    @ivar parent: Parent node, not really used but might be useful later for optimizing AST transformations.
    @ivar data: Any extra data, usually a character to be concatenated, or a duplicating operator
    
    @ivar string: Property shortcut to L{__repr__} because L{compiler.visualize} needs it
    @ivar graph_id: Used in C{compiler.visualize}
    @ivar graph_color: 6-character sharped hex color which will be the node's background color in the graph (e.g. C{#ffffff})
    """
    ...
    def generate_instructions(self, instr_list=None, flags=None):
        ...
        These two lines of code must be at the start of every C{generate_instructions()} implementation::
            
            instr_list = instr_list or []
            flags = flags or {}
        
        @param instr_list: An existing list of instructions. If left blank, a new empty list will be created.
        @param flags: A dict of flags which is passed through all calls to L{ASTNode.generate_instructions}. Used for generating code for matching/nonmatching bracket expressions.
...
class RegexNode(ASTNode):
    ...
    def generate_instructions(self, instr_list=None, flags=None):
        """
        Generate code for a | operator if C{subtype == 'alt'}::
        ...
        If C{subtype != 'alt'}, then this node is reduced away during parsing.
        """
        instr_list = instr_list or []
        flags = flags or {}
        if self.subtype == 'alt':
...
class NonDupReNode(ASTNode):
    ...
    def generate_instructions(self, instr_list=None, flags=None):
        """
        Generate code for a character if C{subtype == 'char'}::
...
class SimpleReNode(ASTNode):
    ...
    def generate_instructions(self, instr_list=None, flags=None):
        """
        Generate code for a +, *, or ? operator if C{subtype == 'dup'}.
...
class EndRangeNode(ASTNode):
    """
    Generates code for characters in a matching or non-matching bracket expression.
    """
...
class CharClassNode(ASTNode):
    """
    Pretends to be a chain of C{follow_list}s, obeys C{matching} flag, complains if the user is doing something weird (see below). This class is a little bit odd with the intention of avoiding duplication of the code in L{BrackExprListNode}.
    """
    ...
    def generate_instructions(self, instr_list=None, flags=None):
        """
        Use C{matching} flag and presence of C{self.data} in char class mapping dicts to determine what kind of instruction to return, and what its character arguments are
        """
...
class BrackExprListNode(ASTNode):
    """Performs transformations and passes down flags in order to implement matching and nonmatching lists."""
    ...
    def generate_instructions(self, instr_list=None, flags=None):
        """
        Perform transformations and pass down flags in order to implement matching and nonmatching lists.
        
        For matching lists, generate a block of C{SPLIT}s, generate children's C{CHAR} instructions, and interleave them between a series of C{JMP}s. Example::
